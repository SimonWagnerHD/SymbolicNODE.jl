...
x_aug = SymbolicAugment(gsr,[3,4];min_size=2, max_size=6)
...
p, re_nn = NODE_ANN(x_aug.N_eqn,2,64,2,activation=tanh)
...
train_losses, valid_losses = train_NODE(model, train_data, epochs; valid_data=valid_data, re_nn=re_nn, η=1f-3, decay=1f-6, print_every=5, save_every=50, savefile="../models/genetic_node_split")

using DelimitedFiles

writedlm(string("../data/genetic_node_split_train",epochs,".csv"), train_losses, ',')
writedlm(string("../data/genetic_node_split_valid",epochs,".csv"), valid_losses, ',')
save_ANN(re_nn(model.p), string("../models/genetic_node_split",epochs,".bson"))