...
function node(du, u, p, t)
    ...
    du[1] = ω₁
    du[2] = ω₂
    du[3:4] .= re_nn(p)(vcat(gsr(u)[3:4],[ω₁, ω₂, sin(θ₁), cos(θ₁), sin(θ₂), cos(θ₂)]))
...
train_losses, valid_losses = train_NODE(model, train_data, epochs; valid_data=valid_data, re_nn=re_nn, η=1f-3, decay=1f-6, print_every=5, save_every=50, savefile="../models/genetic_node_ext")

using DelimitedFiles

writedlm(string("../data/genetic_node_ext_train",epochs,".csv"), train_losses, ',')
writedlm(string("../data/genetic_node_ext_valid",epochs,".csv"), valid_losses, ',')
save_ANN(re_nn(model.p), string("../models/genetic_node_ext",epochs,".bson"))